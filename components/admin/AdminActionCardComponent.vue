    ...
    <n-card
        ...
        <template #header>
            <div class="flex items-center justify-between mb-4">
                ...
                <div
                    ...
                    <Icon :name="data.icon"
                        class="text-emerald-400 transition-transform duration-300 group-hover:scale-110" :size="24" />
                </div>
            </div>
        </template>
        ...
        <template #action>
            <n-space justify="end" align="center">
                <n-button v-if="data.hasSecondary" secondary class="hover:border-emerald-400/50"
                    :render-icon="() => h(NIcon, { name: 'ph:upload-simple-bold' })">
                    {{ data.secondaryButtonButtonContent }}
                </n-button>

                <n-button type="primary" color="#4ade80" :render-icon="() => h(NIcon, { name: 'ph:arrow-right-bold' })"
