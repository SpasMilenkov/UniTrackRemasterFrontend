<template>
    <n-space vertical :size="12" class="min-h-screen md:px-6 px-2 py-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 py-8 px-8 md:px-0">
            <n-card class="dark:hover:bg-[hsl(240,8%,13%)] duration-150 dark:bg-[#18181c] dark:text-white rounded"
                title="Next Class">
                <template #header-extra>
                    <Icon name="material-symbols:school-outline" class="text-emerald-400" :size="36" />
                </template>
                <div class="mt-2">
            ...
            <n-card class="dark:hover:bg-[hsl(240,8%,13%)] duration-150 dark:bg-[#18181c] dark:text-white rounded"
                title="Upcoming Exam">
                <template #header-extra>
                    <Icon name="mdi:file-document-edit-outline" class="text-emerald-400" :size="36" />
                </template>
                <div class="mt-2">
        ...
        <div class="grades-table-wrapper">
            ...
            <div class="mb-4 flex items-center justify-between">
                <div class="flex items-center gap-4">
                    <n-input-group>
                        <n-input v-model:value="searchQuery" placeholder="Search students..." clearable class="w-64">
                            <template #prefix>
            ...
            <div class="overflow-hidden rounded-xl border border-gray-700 bg-[#262629]">
                <n-data-table ref="dataTableRef" :bordered="false" :single-line="false" :columns="columns"
                    :data="filteredData" :pagination="pagination" :loading="loading" class="custom-table" />
            </div>
            ...
            <div class="mt-4 grid grid-cols-3 gap-4">
                <div v-for="stat in statistics" :key="stat.label"
                    ...
                    <div class="mt-1 text-xl font-bold text-emerald-400">{{ stat.value }}</div>
                </div>
            </div>
        </div>

        <!-- Grade Modal -->
        <n-modal :show="showModal" style="width: 600px" title="Add Grades" :bordered="false" size="huge"
...
const generateData = (): RowData[] => {
    ...
    const studentNames = ['Elden Mesho', 'K. Honda', 'Almond Lover', 'Marto Koleff', 'Marto Borisov'];
