<template>
    <div class="min-h-screen bg-[#101014] text-white relative">
            ...
            <div class="py-4">
                <div class="mx-auto max-w-7xl px-6 lg:px-8">
                    <div class="flex flex-col sm:flex-row gap-4 justify-between items-center">
                        <!-- Search -->
                        <n-input-group>
                            <n-input v-model:value="searchQuery" placeholder="Search applications..."
                                class="min-w-[300px]">
                                <template #prefix>
                        ...
                        <!-- Filter by School -->
                        <n-select v-model:value="selectedSchool" :options="schoolOptions" placeholder="Filter by school"
                            class="min-w-[200px]" />
                    </div>
                </div>
            </div>
                ...
                <div class="mx-auto max-w-7xl px-6 lg:px-8">
                    <div class="space-y-4">
                        ...
                        <div v-for="application in filteredApplications" :key="application.id"
                            ...
                            <div class="flex flex-col sm:flex-row justify-between gap-6">
                                    ...
                                    <div class="flex items-start justify-between">
                                        <div>
                                            ...
                                            <div class="space-y-1">
                                                ...
                                                <p class="text-gray-400">Code: {{ application.code }}</p>
                                            </div>
                                        </div>
                                ...
                                <div class="flex items-center gap-3 sm:flex-col lg:flex-row">
                                    <n-button v-if="application.status === 0" type="primary" color="#4ade80"
                                        class="flex-1" @click="approveApplication(application.id)">
                                        <template #icon>
                                            <Icon name="ph:check-bold" />
                                        </template>
                                        Approve
                                    </n-button>
                                    <n-button v-if="application.status === 0" type="error" class="flex-1"
                                    ...
                                    <n-tag v-if="application.status === 1" type="success"
                                        :bordered="false">Approved</n-tag>
                                    <n-tag v-if="application.status === 2" type="error"
                                        :bordered="false">Rejected</n-tag>
                                </div>
                            </div>
                        </div>
                        ...
                        <div v-if="filteredApplications?.length === 0"
                            ...
                            <Icon name="ph:folder-simple" class="text-gray-400 text-5xl mb-4" />
        ...
        <!-- Rejection Dialog -->
        <n-modal v-model:show="showRejectModal" size="medium" :mask-closable="false"
            class="w-[90vw] max-w-xl h-auto max-h-[80vh]">
            <div class="bg-[#18181c] border border-gray-700/50 rounded-xl p-6 text-white">
                ...
                <div class="mb-6">
                    <label for="rejection-reason" class="block text-gray-400 mb-2">Reason for Rejection</label>
                    <n-input id="rejection-reason" v-model:value="rejectionReason" type="textarea"
...
const schoolOptions = computed(() => {
    const uniqueSchools = new Set(schoolOnboardingStore.applications?.map(app => app.schoolName))
...
const filteredApplications = computed(() => {
    return schoolOnboardingStore.applications?.filter(app => {
        ...
        const matchesSearch = searchQuery.value ?
            searchFields.includes(searchQuery.value.toLowerCase()) : true

        const matchesSchool = selectedSchool.value ?
            app.schoolName === selectedSchool.value : true
...
const approveApplication = async (applicationId: string) => {
    ...
    try {
        const currentApplication = schoolOnboardingStore.applications?.find(a => a.id === applicationId);
